...
class LinkTableGenerator():
    def __init__(self, link):
        ...
        for hr in self.link.linkhubreference_set.all():
            if hr.hub_alias is None or hr.hub_alias == '':
                self.field_list.append(hr.hub.get_hash_key_field)
            else:
                self.field_list.append(hr.hub.get_hash_key_field)
                
        
        for f in link.linkfield_set.all():
